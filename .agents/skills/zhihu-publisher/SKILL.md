...
description: Publish Markdown articles to Zhihu Zhuanlan (知乎专栏) using playwright-cli automation. Use when the user wants to publish articles to Zhihu, especially when they provide a Markdown file path. Handles title, content, and publication settings automatically.
---
...
**Topics are not required for publishing.** If the "发布" button is already enabled (not disabled), you can skip this step entirely and proceed directly to Step 7.

If you want to add topics, try clicking the suggested topic buttons that appear below the topic input:

```bash
playwright-cli snapshot
# Look for suggested topic buttons like "AI", "AI技术", "AI芯片" at the bottom
# Click one directly
playwright-cli click <suggested_topic_ref>
```

To search for a topic (may not work reliably — Zhihu's React search input doesn't always respond to keyboard events):

```bash
# Click "添加话题" button
playwright-cli eval "[...document.querySelectorAll('button')].find(btn => btn.textContent.includes('添加话题'))?.click()"

# Try searching (may not show results)
playwright-cli fill <topic_search_ref> "AI"
playwright-cli press "Enter"

# If no results appear, dismiss the dialog and skip topics
playwright-cli press "Escape"
```

**Known issue**: The topic search input often does not trigger search results when using `playwright-cli press "Enter"` (Zhihu's React autocomplete doesn't respond to Playwright keyboard events). If search fails, skip topics — publishing works without them.
