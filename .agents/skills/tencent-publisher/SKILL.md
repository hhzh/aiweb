---
name: tencent-publisher
description: Publish Markdown articles to Tencent Cloud Developer Community using playwright-cli automation. Use when the user wants to publish articles to Tencent Cloud, especially when they provide a Markdown file path. Handles title, content, tags, and publication settings automatically.
---
...
**CRITICAL**: The tag system only accepts **predefined tags from the dropdown**. Custom tags (e.g., "Codex", "子代理") will NOT be added when you type and press Enter — they silently fail. Type a keyword, wait for the dropdown, then select from the available options.
...
**Do NOT** waste time trying to add custom tags like "Codex", "AI编程", "子代理" — they will not work.
