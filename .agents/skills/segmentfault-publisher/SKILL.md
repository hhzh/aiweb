...
playwright-cli eval 'Array.from(document.querySelectorAll(".tab-pane.active a")).find(function(a){return a.textContent.trim()==="Codex"}).click()'
```

**CRITICAL**: Tag links are `<a class="badge-tag m-1" href="###">` elements. Broad category tags (人工智能, 机器学习, etc.) will navigate away from the editor when clicked, triggering a beforeunload dialog. Only use sub-tags (chatgpt, openai, prompt, Codex, llm, etc.) which correctly add to the article. Also, `playwright-cli eval` does NOT support multi-line JS — use single-line expressions only.
...
- Page title should contain the article title and blog name (e.g., "Codex 指南 - 小林学AI - SegmentFault 思否")
...
- **Fix**: Use JS eval (single-line) to click only sub-tags (chatgpt, openai, prompt, Codex, etc.), not broad category tags
