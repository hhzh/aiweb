...
### Radio Button 必须用 playwright-cli click（CRITICAL）

思否的表单验证监听原生鼠标事件，**radio 按钮必须用 `playwright-cli click` 触发**，不能用 JS eval click：

```bash
# ✅ CORRECT — triggers form validation
playwright-cli click '#formHorizontalRadios1'

# ❌ WRONG — does NOT trigger form validation (button stays disabled)
playwright-cli eval "document.querySelector('#formHorizontalRadios1')?.click()"
```

如果提交按钮始终保持 disabled，检查验证错误提示：
```bash
playwright-cli eval "Array.from(document.querySelectorAll('.text-danger, .invalid-feedback, .error, [class*=error], [class*=invalid]')).filter(function(el){return el.innerText.trim().length>0}).map(function(el){return el.innerText.trim()})"
```

常见验证失败原因：`请选择文章类型` — radio 未通过原生事件触发。
