...
Hermes Agent 的配置核心围绕 **模型选择、执行环境、记忆能力、工具集成** 四大模块，所有配置集中在 `~/.hermes/` 目录（`config.yaml` 存普通配置、`.env` 存密钥），支持交互式向导与手动编辑两种方式，兼顾新手易用性与进阶灵活性。本文从快速配置、核心模块详解、常用示例、问题排查四方面，带你完成全流程配置。
