...
想让 Agent 帮你操作电脑，又怕它抢走鼠标键盘、干扰手头工作？Hermes Agent 电脑操控（Computer Use）正是为此而生——一项**后台桌面自动化能力**，可在不干扰用户当前操作的前提下，实现 Mac 桌面的点击、输入、滚动、拖拽等操作，让 Agent 与你协同工作。本文从核心原理、安装配置、基础用法、安全机制到性能优化，带你全面掌握电脑操控功能。
...
```mermaid
flowchart LR
    Agent[Hermes Agent] -->|MCP 协议| Driver[cua-driver<br/>开源驱动]
    Driver -->|macOS 私有 SPI| Events[后台合成事件]
    Events -->|点击 / 输入 / 滚动 / 拖拽| Target[目标进程]
    Target -->|无障碍树| Driver
    Driver -->|截图 + SOM 标注| Agent
```
