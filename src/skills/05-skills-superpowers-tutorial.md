...
AI 编码工具生成代码很快，但缺乏工程规范——不写测试、不改再审查、随意重构无关代码。Superpowers 通过可组合的 Skills 技能集，让 Claude、Cursor、Codex、Copilot CLI、Gemini 等工具具备系统化开发能力，无需手动干预即可遵循规范流程完成开发任务。

## 核心认知：Superpowers 与 Skills 是什么？

在安装使用之前，先理解 Superpowers 的定位与运行逻辑，有助于后续正确选用技能。
...
安装完成后，所有 Skills 均为强制工作流，AI 智能体执行开发任务时会自动触发，无需手动调用。
...
7 步闭环之外，Superpowers 还提供测试、调试、协作、元技能四大类技能，按需组合。
...
以下通过一个完整案例，演示 Superpowers 从安装到交付的全流程。
...
Superpowers 的核心价值是给 AI 编码工具注入工程素养——通过标准化的技能集和强制流程，解决"无序、跑偏、质量差"的痛点。完成对应平台安装后，即可享受全流程自动化的规范开发体验。
