...
你是否每次使用 Claude Code 都要重复输入同样的审查清单？或者团队每个人都各自写一套部署流程，无法统一？Skills 正是为解决这些重复性问题而生的——它将高频操作、团队规范、专项任务固化为可调用的技能，支持 `/技能名` 手动触发或对话自动匹配，实现”一次配置，终身复用”。本文从基础入门、实操创建、高级功能到问题排查，结合完整代码示例，帮你掌握 Skills 的使用。
...
创建完第一个技能后，接下来看技能放在哪里——存储路径决定适用范围与共享权限，同时需遵循标准目录结构确保正常加载。
...
明确了路径与结构，接下来深入 SKILL.md 的配置细节——这是技能的核心文件，分为 YAML 元数据与 Markdown 执行指令两部分。
...
掌握基础创建后，以下高级功能可让技能更灵活、更强大——参数传递实现动态执行，动态上下文注入获取实时数据，子代理隔离避免上下文污染。
...
高级功能带来更强的能力，但也需要更精细的权限管控——本节覆盖调用权限控制、工具权限限制和全局权限管控。
...
掌握了创建、配置与权限管理，以下是使用过程中的高频问题及解决方案。
...
**2. 确认技能已被加载**：在 Claude Code 中输入 `/` 查看可用技能列表，确认目标技能出现在列表中。
...
**1. 确认 `$ARGUMENTS` 占位符存在**：检查 SKILL.md 中是否包含 `$ARGUMENTS`，且拼写正确。

**2. 调用格式正确**：参数紧跟技能名，无多余空格（多个参数用空格分隔）。
...
**2. 移出非核心内容**：将详细示例、参考文档等移至 `reference.md` 或 `examples.md`，按需加载。
...
**实操建议**：从高频重复操作入手（如代码审查、格式美化），创建简单技能，熟悉配置规则后再尝试高级功能；团队协作时，将项目规范固化为项目技能，通过 Git 共享，实现团队操作统一。

**下一步方向**：尝试将团队现有的 Shell 脚本、部署流程迁移为 Skills，体验”一次配置，终身复用”的效率提升；进阶可探索 Skills 与 Plugins、Subagents 的组合使用，构建更完整的自动化工作流。
