...
4. **问题1：上下文臃肿，模型遗忘指令**原因：单次请求加载过多文件、无关任务混用。解决方案：拆分任务、及时清理上下文，用 Subagents 分担调研任务。`// 错误：一次加载多个无关文件
...
5. **问题2：CLAUDE.md 臃肿，模型忽略规则**原因：包含过多可推导信息、通用规范，篇幅过长。解决方案：精简至核心规则，非通用内容移入 Skills。`# 错误：CLAUDE.md 包含通用 JavaScript 教程
# 正确：仅保留项目专属规则，通用规范链接外部文件
> 通用 JavaScript 规范参考：@docs/JS规范.md
``> 项目专属规则：（仅保留差异化内容）`

6. **问题3：自动化任务执行失败，无报错提示**原因：未指定输出格式、未添加错误处理。解决方案：指定输出格式，添加错误捕获与提示。`// 推荐：添加错误处理，指定输出格式
