...
`/goal` 是 Claude Code 的会话级自动化命令，核心作用是**设定明确的目标完成条件**。触发后，Claude 会自主推进任务。每轮工作结束后，由独立的轻量评估模型（默认 Haiku）验证条件是否满足。未满足则自动开启下一轮，满足则立即停止并清除目标，全程无需人工干预。
...
`/goal` 是**条件驱动的自主循环**，Auto mode 是**单轮内的自动化审批**，二者可搭配使用，实现 “无人工干预的全流程自主任务处理”。
