...
# Claude Code subagent 子代理

你是否遇到过这样的场景：让 Claude Code 同时审查代码和调试报错，结果上下文塞满了审查结果，调试时关键信息已被压缩丢失？或者主会话越来越慢，因为之前读取的大量文件占据了上下文？子代理正是为解决这些问题而生——它拥有独立上下文，不污染主对话，可并行处理专项任务，是复杂开发场景下的提效关键。
...
理解了子代理的核心定位后，先看 Claude Code 自带了哪些子代理——无需手动配置，主代理会根据任务自动调用。
...
内置子代理满足通用场景，但项目专属需求（如前端规范审查、后端调试）需要自定义子代理。以下3种方式覆盖新手到进阶用户。
...
创建自定义子代理时，放在哪里决定了谁可以使用——作用域与优先级规则避免配置冲突。
...
明确了作用域，接下来看配置文件中的具体字段——哪些必选、哪些按需，避免过度配置。
...
掌握基础配置后，以下高级能力可进一步提升开发效率——主代理与子代理的协同模式、手动调用与参数传递、上下文管理。
...
子代理的核心价值是"独立上下文+专项分工"——通过隔离执行环境避免主会话污染，通过专业化配置提升输出质量，通过并行处理加速任务完成。建议从团队共享的代码审查子代理入手，熟悉配置规则后，再尝试创建调试、安全审查等专项子代理；复杂任务可采用链式协同或并行协同模式，最大化提升效率。
