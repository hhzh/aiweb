...
"代码式"绘图听起来劝退？Mermaid 的语法简单到让你惊呼"So easy!"。
...
了解了 Mermaid 支持的图表类型后，流程图是使用频率最高的，以下从基础到进阶逐步讲解。
...
流程图描述"做什么"，时序图则描述"谁在什么时候对谁说了什么"——多角色交互场景的首选。
...
时序图聚焦交互，甘特图则聚焦时间——展示任务的排期、时长和进度，适合项目管理和内容日历。
...
甘特图管理时间，类图则梳理概念——展示对象间的继承、组合、依赖关系。
...
掌握了各类图表语法后，渲染失败是最常见的拦路虎——99% 是语法问题，以下排查流程帮你快速定位。
