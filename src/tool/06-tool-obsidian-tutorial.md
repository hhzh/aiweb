...
用 AI 管理知识库，通用提示词要么不懂 Obsidian 双链语法，要么无法操作笔记——obsidian-skills 是 Obsidian 官方适配的 AI 智能体技能库，让 Claude Code、Codex CLI、OpenCode 等工具精准理解 Obsidian 的专有语法、画布、数据库与命令行，以原生方式读写笔记、管理知识库、开发插件。
...
5 项技能中，obsidian-cli 是能力核心——让 AI 智能体通过命令行直接与 Obsidian 交互，无需打开 GUI，实现自动化、批量化操作。
...
CLI 负责操作，obsidian-markdown 则确保 AI 生成的内容符合 Obsidian 专有规范——双链、标注、元数据缺一不可。
...
CLI 和 Markdown 技能掌握后，以下 3 个案例展示 AI 与 Obsidian 协作的典型场景。
...
obsidian-skills 的核心是 obsidian-cli——它让 AI 从”只能读写文本”升级为”原生管理 Obsidian 知识库”。建议先安装 CLI 技能，用 `obsidian search` + `obsidian new` 体验自动维护知识库，再结合 obsidian-markdown 确保生成内容符合 Obsidian 规范。
