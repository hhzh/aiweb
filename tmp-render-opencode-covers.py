...
ARTICLES = {
    "01-opencode-quick-start-guide": {
        "title": "快速入门教程",
        "eyebrow": "OpenCode · Quick Start",
        "subtitle": "从安装、基础配置到第一个项目，30 分钟快速上手开源 AI 编码代理。",
        "theme": "config",
        "chips": ["安装 OpenCode", "基础配置", "命令行入门", "第一行代码"],
        "lines": ["opencode init", "opencode start", "model selected", "ready"],
        "palette": ((7, 18, 32), (9, 105, 218), (34, 211, 238), (52, 211, 153)),
    },
    "02-opencode-commands-guide": {
        "title": "命令使用教程",
        "eyebrow": "OpenCode · Commands",
        "subtitle": "覆盖 CLI 命令、会话控制、文件操作与调试指令，掌握核心交互。",
        "theme": "workflow",
        "chips": ["会话命令", "调试指令", "文件操作", "/ 快捷指令"],
        "lines": ["opencode chat", "opencode plan", "opencode review", "opencode run"],
        "palette": ((9, 19, 36), (79, 70, 229), (14, 165, 233), (250, 204, 21)),
    },
    "03-opencode-tools-guide": {
        "title": "工具使用教程",
        "eyebrow": "OpenCode · Tools",
        "subtitle": "Bash、文件编辑、搜索、网页抓取……解码内置工具链的正确用法。",
        "theme": "mcp",
        "chips": ["Bash", "文件编辑", "Grep", "Web 抓取"],
        "lines": ["open tool bash", "open tool edit", "open tool search", "tool ready"],
        "palette": ((13, 24, 40), (147, 51, 234), (45, 212, 191), (251, 146, 60)),
    },
    "04-opencode-skills-guide": {
        "title": "Skills 使用教程",
        "eyebrow": "OpenCode · Skills",
        "subtitle": "把专项技能、工作流和参考资料封装为按需加载的 Skill 包。",
        "theme": "skills",
        "chips": ["SKILL.md", "技能市场", "自定义开发", "按需加载"],
        "lines": ["load skill env", "skill registered", "merge rules", "apply skill"],
        "palette": ((8, 27, 34), (5, 150, 105), (14, 165, 233), (250, 204, 21)),
    },
    "05-opencode-plugins-guide": {
        "title": "插件使用教程",
        "eyebrow": "OpenCode · Plugins",
        "subtitle": "监听事件、拦截工具、注入逻辑、集成外部服务——用插件无限扩展。",
        "theme": "plugins",
        "chips": ["事件监听", "工具拦截", "NPM 包", "本地文件"],
        "lines": ["plugin installed", "hook attached", "event captured", "plugin ready"],
        "palette": ((18, 22, 39), (217, 70, 239), (14, 165, 233), (245, 158, 11)),
    },
    "06-opencode-model-switching-guide": {
        "title": "切换模型教程",
        "eyebrow": "OpenCode · Model Switching",
        "subtitle": "多模型自由切换，按任务复杂度选择性价比最优的推理引擎。",
        "theme": "config",
        "chips": ["模型配置", "Provider", "自动切换", "本地模型"],
        "lines": ["switch model", "provider set", "model online", "ready"],
        "palette": ((6, 24, 43), (37, 99, 235), (16, 185, 129), (168, 85, 247)),
    },
    "07-opencode-rules-guide": {
        "title": "Rule 使用教程",
        "eyebrow": "OpenCode · Rules",
        "subtitle": "用层级规则引擎控制工具执行权限，兼顾安全与效率。",
        "theme": "rules",
        "chips": ["全局规则", "项目规则", "匹配策略", "安全管控"],
        "lines": ["rule matched", "permission ok", "block denied", "audit log"],
        "palette": ((19, 23, 35), (220, 38, 38), (245, 158, 11), (45, 212, 191)),
    },
    "08-opencode-permissions-guide": {
        "title": "权限配置教程",
        "eyebrow": "OpenCode · Permissions",
        "subtitle": "控制每条命令的执行权限，防范未授权操作，适配团队协作。",
        "theme": "agents",
        "chips": ["allow 模式", "禁止列表", "白名单", "灰度规则"],
        "lines": ["allow: read", "prompt: write", "forbidden: rm", "policy set"],
        "palette": ((12, 22, 42), (79, 70, 229), (34, 211, 238), (132, 204, 22)),
    },
    "09-opencode-agent-guide": {
        "title": "Agent 使用教程",
        "eyebrow": "OpenCode · Agent",
        "subtitle": "用专用子代理并行处理审查、调试、测试与文档生成等任务。",
        "theme": "subagents",
        "chips": ["专用代理", "并行执行", "上下文隔离", "结果合并"],
        "lines": ["spawn agent", "assign task", "wait result", "merge answer"],
        "palette": ((10, 24, 42), (8, 145, 178), (168, 85, 247), (34, 197, 94)),
    },
    "10-opencode-ui-ide-integration-guide": {
        "title": "UI 与 IDE 集成",
        "eyebrow": "OpenCode · UI & IDE",
        "subtitle": "TUI 终端、Web 界面、VS Code、Cursor——全场景覆盖。",
        "theme": "custom",
        "chips": ["TUI 界面", "Web 面板", "VS Code", "Cursor"],
        "lines": ["opencode ui", "web connected", "ide plugin active", "ready"],
...
def draw_title(d, meta):
    d.text((88, 208), "OpenCode", font=F_TITLE, fill=(139, 233, 253, 255))
...
def draw_terminal(img, meta, palette):
    ...
    d.text((1008, y), "opencode", font=F_MONO, fill=(248, 250, 252, 255))
    y += 52
...
def draw_icon(img, meta, palette):
    ...
    if theme in {"config", "examples"}:
        ...
        nodes = [("Rules", cx, cy - 78), ("Plugins", cx - 120, cy + 48), ("Skills", cx + 120, cy + 48)]
